...
durchgesetzt haben, die Objekterkennung als einzelnes Regressionsproblem
formulieren und dadurch Bounding Boxes und Klassenwahrscheinlichkeiten in
einem einzigen Netzwerkdurchlauf vorhersagen @redmon2016yolo. Ein zentrales
Merkmal solcher Ansätze ist jedoch, dass sie auf einem festen, vorab
definierten Satz an Objektklassen trainiert werden. Sollen neue oder
projektspezifische Objekte erkannt werden, ist ein erneutes Training bzw.
Finetuning mit entsprechend annotierten Daten notwendig.
...
Foundation Models basieren und darauf abzielen, die feste
Kategorienbindung klassischer Detektoren zu überwinden @li2026hdino.
Diese Modelle koppeln Bild- und Textrepräsentationen und erlauben es,
Objekte anhand einer natürlichsprachlichen Beschreibung zu lokalisieren,
ohne dass eine klassenspezifische Trainingsphase erforderlich ist.
Ergänzend existieren Segmentierungsmodelle, die ausgehend von groben
Lokalisierungen wie Bounding Boxes präzise Objektkonturen extrahieren
können. Die Kombination beider Modelltypen ermöglicht eine flexible,
promptgesteuerte Objekterkennung mit anschließender pixelgenauer
Segmentierung, ohne dass für jedes neue Zielobjekt ein eigener
Trainingsprozess nötig ist.

Die Wahl zwischen klassentrainierten und promptbasierten Verfahren stellt
somit einen Trade-off dar. Klassische, trainierte Detektoren
bieten in der Regel eine höhere Verarbeitungsgeschwindigkeit und
Genauigkeit auf bekannten Klassen, benötigen dafür jedoch annotierte
Trainingsdaten @heritage2026. Promptbasierte Foundation-Model-Ansätze bieten hingegen
hohe Flexibilität bei neuen Zielobjekten, sind jedoch typischerweise
rechenintensiver und stärker von der Formulierung des Prompts sowie den
